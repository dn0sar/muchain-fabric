...
func (impl *StateImpl) Get(chaincodeID string, key string) ([]byte, error) {
	compositeKey := stcomm.ConstructCompositeKey(chaincodeID, key)
...
func (impl *StateImpl) AddChangesForPersistence(writeBatch *gorocksdb.WriteBatch) error {
	...
	for _, updatedChaincodeID := range updatedChaincodeIds {
		...
		for updatedKey, value := range updates {
			compositeKey := stcomm.ConstructCompositeKey(updatedChaincodeID, updatedKey)
			if value.IsDeleted() {
...
func (impl *StateImpl) GetStateSnapshotIterator(snapshot *gorocksdb.Snapshot) (stcomm.StateSnapshotIterator, error) {
...
func (impl *StateImpl) GetRangeScanIterator(chaincodeID string, startKey string, endKey string) (stcomm.RangeScanIterator, error) {
	panic("Not a full-fledged state implementation. Implemented only for measuring best-case performance benchmark")
}
