...
func (itr *RangeScanIterator) Next() bool {
	...
	for ; itr.dbItr.Valid(); itr.dbItr.Next() {
		...
		trieKeyBytes := stcomm.Copy(itr.dbItr.Key().Data())
		trieNodeBytes := stcomm.Copy(itr.dbItr.Value().Data())
		...
		// found an actual key
		currentCompositeKey := trieKeyEncoderImpl.decodeTrieKeyBytes(stcomm.Copy(trieKeyBytes))
		currentChaincodeID, currentKey := stcomm.DecodeCompositeKey(currentCompositeKey)
