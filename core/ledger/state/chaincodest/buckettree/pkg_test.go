...
func (testHasher *testHasher) populate(chaincodeID string, key string, hash uint32) {
	testHasher.testHashFunctionInput[string(stcomm.ConstructCompositeKey(chaincodeID, key))] = hash
}
...
func (testWrapper *stateImplTestWrapper) getRangeScanIterator(chaincodeID string, startKey string, endKey string) stcomm.RangeScanIterator {
