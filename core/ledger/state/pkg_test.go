...
func createFreshDBAndConstructState(t *testing.T) (*stateTestWrapper, *chaincode_state.State) {
...
func newStateTestWrapper(t *testing.T) *stateTestWrapper {
	return &stateTestWrapper{t, chaincode_state.NewState()}
}
...
func (testWrapper *stateTestWrapper) fetchStateDeltaFromDB(blockNumber uint64) *chaincode_state.StateDelta {
	delta := chaincode_state.NewStateDelta()
