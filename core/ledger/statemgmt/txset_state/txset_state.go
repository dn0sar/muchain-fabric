...
var txSetLogger = logging.MustGetLogger("txset_state")
...
func NewTxSetState() *TxSetState {
	confData := state_comm.GetConfig("txSetState", defaultTxSetStateImpl, buckettreeType, trieType, rawType)
	txSetLogger.Infof("Initializing tx set state implementation [%s]", confData.StateImplName)
	switch txSetStateImplType(confData.StateImplName) {
	case buckettreeType:
		txSetStateImpl = buckettree.NewStateImpl()
	case trieType:
		txSetStateImpl = trie.NewStateImpl()
	case rawType:
		txSetStateImpl = raw.NewStateImpl()
	default:
...
func (state *TxSetState) TxBegin(txID string) {
	txSetLogger.Debugf("txBegin() for txId [%s]", txID)
...
func (state *TxSetState) TxFinish(txID string, txSuccessful bool) {
	txSetLogger.Debugf("txFinish() for txId [%s], txSuccessful=[%t]", txID, txSuccessful)
	...
	if txSuccessful {
		if !state.currentTxStateDelta.IsEmpty() {
			txSetLogger.Debugf("txFinish() for txId [%s] merging state changes", txID)
...
func (state *TxSetState) Set(txSetID string, index uint32) error {
	txSetLogger.Debugf("set() txSetID=[%s], key=[%s], index=[%#v]", txSetID, index)
	...
	if state.currentTxStateDelta.IsUpdatedValueSet(txSetID) {
		txSetLogger.Warning("Potential dependency cycle avoided by not changing an already modified tx set index")
...
func (state *TxSetState) Delete(txSetID string) error {
	txSetLogger.Debugf("delete() txSetID=[%s]", txSetID)
	if !state.txInProgress() {
		panic("State can be changed only in context of a tx.")
	}
...
func (state *TxSetState) GetHash() ([]byte, error) {
	txSetLogger.Debug("Enter - GetHash()")
	if state.updateStateImpl {
		txSetLogger.Debug("updating stateImpl with working-set")
	...
	txSetLogger.Debug("Exit - GetHash()")
	return hash, nil
}
...
func (state *TxSetState) ClearInMemoryChanges(changesPersisted bool) {
	state.txSetStateDelta = statemgmt.NewStateDelta()
...
func (state *TxSetState) AddChangesForPersistence(blockNumber uint64, writeBatch *gorocksdb.WriteBatch) {
	txSetLogger.Debug("state.addChangesForPersistence()...start")
	if state.updateStateImpl {
		state.txSetStateImpl.PrepareWorkingSet(state.txSetStateDelta)
		state.updateStateImpl = false
	}
	...
	txSetLogger.Debugf("Adding state-delta corresponding to block number[%d]", blockNumber)
	...
	if blockNumber >= state.historyStateDeltaSize {
		...
		txSetLogger.Debugf("Deleting state-delta corresponding to block number[%d]", blockNumberToDelete)
		writeBatch.DeleteCF(cf, state_comm.EncodeStateDeltaKey(blockNumberToDelete))
	} else {
		txSetLogger.Debugf("Not deleting previous state-delta. Block number [%d] is smaller than historyStateDeltaSize [%d]",
			blockNumber, state.historyStateDeltaSize)
	}
	txSetLogger.Debug("state.addChangesForPersistence()...finished")
}
...
func (state *TxSetState) ApplyStateDelta(delta *statemgmt.StateDelta) {
...
func (state *TxSetState) DeleteState() error {
	...
	if err != nil {
		txSetLogger.Errorf("Error deleting state: %s", err)
	}
	return err
}
