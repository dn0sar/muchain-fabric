...
func TestLedgerCommit(t *testing.T) {
	...
	ledger.ChainTxFinished("txUuid", true)
...
func TestLedgerGetTempStateHashWithTxDeltaStateHashes(t *testing.T) {
	...
	ledger := ledgerTestWrapper.ledger
	ledger.BeginTxBatch(1)
	ledger.ChainTxBegin("txUuid1")
	ledger.SetState("chaincode1", "key1", []byte("value1"))
	ledger.ChainTxFinished("txUuid1", true)

	ledger.ChainTxBegin("txUuid2")
	ledger.SetState("chaincode2", "key2", []byte("value2"))
	ledger.ChainTxFinished("txUuid2", true)

	ledger.ChainTxBegin("txUuid3")
	ledger.ChainTxFinished("txUuid3", true)

	ledger.ChainTxBegin("txUuid4")
	ledger.SetState("chaincode4", "key4", []byte("value4"))
	ledger.ChainTxFinished("txUuid4", false)
...
func TestVerifyChain(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		ledger.ChainTxBegin("txUuid" + strconv.Itoa(i))
		ledger.SetState("chaincode"+strconv.Itoa(i), "key"+strconv.Itoa(i), []byte("value"+strconv.Itoa(i)))
		ledger.ChainTxFinished("txUuid"+strconv.Itoa(i), true)
		transaction, _ := buildTestTx(t)
